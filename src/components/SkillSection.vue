...
import imgMongoSvg from "../assets/mongodb-svgrepo-com.svg?url";
...
const techSvgs = {
  ...
  vue: {
    ...
    paths: [
      {
        d: "M24,1.61H14.06L12,5.16L9.94,1.61H0L12,22.39L24,1.61Z",
        fill: "#4FC08D",
      },
      {
        d: "M12,18.49L3.44,4.05H8.53L12,10.08L15.47,4.05H20.56L12,18.49Z",
        fill: "#35495E",
      },
    ],
  },
  ...
  iot: {
    ...
    paths: [
      {
        d: "M5 13v9H3v-9Zm18 0v2h-2v7h-2v-7h-2v-2Zm-11-2a7.537 7.537 0 0 1 3.96 1.149l1.447-1.45A9.522 9.522 0 0 0 12 9a9.363 9.363 0 0 0-5.333 1.68l1.449 1.453A7.36 7.36 0 0 1 12 11Z",
        fill: "#0091EA",
      },
      {
      ...
      {
        d: "M12 3a15.471 15.471 0 0 1 9.687 3.41l1.427-1.429A17.43 17.43 0 0 0 .96 4.964l1.427 1.429A15.328 15.328 0 0 1 12 3Zm0 10a4.5 4.5 0 1 0 4.5 4.5A4.5 4.5 0 0 0 12 13Zm0 7a2.5 2.5 0 1 1 2.5-2.5A2.5 2.5 0 0 1 12 20Z",
        fill: "#0091EA",
      },
    ],
  },
  ...
  mongodb: {
    ...
    paths: [
      ...
      {
        d: "M37.084 123.676C37.084 123.676 70.213 101.897 62.461 56.587C54.987 23.643 37.36 12.846 35.423 8.693C33.3 5.74 31.27 0.572 31.27 0.572L32.655 92.207C32.655 92.3 29.794 120.261 37.084 123.676",
        fill: "#499D4A",
      },
      // left leaf (lighter green)
      {
        d: "M29.333 124.875C29.333 124.875 -1.767 103.65 0.079 66.277C1.832 28.903 23.795 10.539 28.04 7.216C30.809 4.264 30.901 3.156 31.086 0.203C33.024 4.356 32.655 62.309 32.931 69.137C33.762 95.437 31.455 119.893 29.333 124.875",
        fill: "#58AA50",
      },
      // stem (grey)
      {
        d: "M35.053 142.317L31.27 141.024C31.27 141.024 31.732 121.738 24.811 120.354C20.197 115.001 25.549 -106.659 42.159 119.615C42.159 119.615 36.438 122.476 35.423 127.367C34.315 132.166 35.053 142.317 35.053 142.317",
        fill: "#A6A385",
      },
    ],
  },
  ...
  ctx = gsap.context(() => {
    ...
    mm.add("(max-width: 767px)", () => {
      ...
      document.querySelectorAll(".curve-card").forEach((card, i) => {
        ...
        const brandColor =
          skill.name === "Express" ? "#a1a1aa" : skill.icon.color || "#10b981";
  ...
  <section id="skills" class="py-16 sm:py-20 md:py-24 transition-colors">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="relative z-10 text-center mb-10 sm:mb-14 md:mb-16">
        <span
          class="inline-block px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-widest bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-200 border border-emerald-300/70 dark:border-emerald-700/50 mb-3"
        >
          Tech Stack
        </span>
        <h2
      ...
      <!-- ── Mobile: Curved path ── -->
      <div
        class="flex md:hidden relative w-full flex-col justify-center items-center z-10 px-2 mt-4 pb-24 overflow-visible"
      >
        <!-- Static glow — no GSAP parallax -->
        <div
          class="absolute top-20 left-1/2 -translate-x-1/2 w-48 h-48 sm:w-64 sm:h-64 bg-emerald-500/15 dark:bg-emerald-600/20 rounded-full blur-[60px] pointer-events-none"
        ></div>

        <!-- Neural line SVG -->
        <div
          class="absolute inset-0 z-0 pointer-events-none flex justify-center mt-16 pb-16"
        >
          <svg
        ...
        <!-- Cards -->
        <div
          class="mobile-curve-track w-full flex flex-col items-center gap-14 relative py-16"
        >
          <div
            ...
            :style="{
              transform: `translateX(${Math.sin((index / (skills.length - 1)) * Math.PI * 3.5) * 72}px)`,
            }"
          >
            <div
              class="w-9 h-9 flex items-center justify-center pointer-events-none"
            >
              <img
                v-if="skill.img"
                :src="skill.img"
            ...
            <span
              class="skill-label absolute -bottom-9 whitespace-nowrap text-[11px] font-bold px-2.5 py-1 rounded-full pointer-events-none font-sans border"
            >
              {{ skill.name }}
            </span>
          </div>
      ...
      <div
        class="hidden md:block client-gallery relative"
        style="height: 480px; perspective: 1100px; overflow: visible"
      >
        ...
        >
          <div
            class="w-[380px] h-[380px] bg-emerald-500/15 dark:bg-emerald-500/20 rounded-full blur-[90px] skill-blob"
          ></div>
        </div>

        <!-- gallery-container fills the exact 480px box, centers the track -->
        <div
          class="absolute inset-0 gallery-container"
          style="transform-style: preserve-3d"
        >
          <div
            class="absolute inset-0 flex items-center justify-center gallery-track"
            style="transform-style: preserve-3d"
          >
            <div
            ...
            >
              <div class="card-floater">
                <div
                  class="relative p-5 md:p-6 w-36 h-28 md:w-40 md:h-32 flex items-center justify-center glass-card rounded-2xl shadow-lg pointer-events-auto hover:scale-110 hover:shadow-xl hover:z-50 transition-transform duration-300 cursor-default"
                >
                  <img
                    v-if="skill.img"
                    :src="skill.img"
                  ...
                  <span
                    class="text-xs font-bold text-emerald-600 dark:text-emerald-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200 absolute -bottom-8 bg-white/90 dark:bg-zinc-800/90 px-3 py-1 rounded-full shadow backdrop-blur-sm whitespace-nowrap border border-emerald-100 dark:border-emerald-800"
                  >
                    {{ skill.name }}
                  </span>
                </div>
...
@media (min-width: 640px) {
  .client-card {
    transform: rotateY(calc(var(--index) * var(--angle))) translateZ(380px);
  }
}
